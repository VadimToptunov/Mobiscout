...
class EnhancedTrainingDataGenerator:
    ...
    def generate_enhanced_selector_data(self) -> Tuple[List[Dict], List[str]]:
        ...
        id_elements = [
            ...
            *self._generate_elements(
                "button", True, True, 150, "id", ["login", "signup", "submit", "payment", "checkout", "confirm"]
            ),
            # Form inputs
            *self._generate_elements(
                "textfield", True, True, 120, "id", ["email", "password", "username", "phone", "address"]
            ),
            # Navigation buttons
            *self._generate_elements("button", True, True, 80, "id", ["home", "search", "profile", "settings", "menu"]),
        ]
        ...
        accessibility_elements = [
            ...
            *self._generate_elements(
            ...
            # Icons and images
            *self._generate_elements(
                "image", True, False, 80, "accessibility_id", ["icon_user", "icon_settings", "avatar", "logo"]
            ),
            # Links
            *self._generate_elements(
                "link", True, True, 70, "accessibility_id", ["help_link", "terms_link", "privacy_link"]
            ),
        ]
    ...
    def _generate_elements(
        self, elem_type: str, visible: bool, enabled: bool, count: int, selector_type: str, prefixes: List[str]
    ) -> List[Dict]:
            ...
            items.append(
                {
                    ...
                    "accessibility_id": "",
                    "xpath": (
                        f"//RecyclerView/ViewHolder[{i}]" if random.random() > 0.5 else f"//ul[@class='items']/li[{i}]"
                    ),
                    "label": f"Item {i}",
            ...
            items.append(
                {
                    ...
                    "accessibility_id": "",
                    "xpath": (
                        f"//button[contains(text(), '{text}')]" if elem_type == "button" else f"//a[text()='{text}']"
                    ),
                    "label": text,
                    "type": elem_type,
                    "visible": True,
    ...
    def generate_enhanced_element_scoring_data(self) -> Tuple[List[Dict], List[float]]:
        ...
        critical = self._generate_scored_elements(
            150,
            0.95,
            1.0,
            ["button"],
        ...
        high = self._generate_scored_elements(
            150,
            0.80,
            0.94,
            ["button", "textfield"],
            ["Search", "Add to Cart", "Email", "Password"],
            {"frequently_used": True, "required": True},
        )

        # Medium importance (0.50-0.79) - 120 samples
        medium = self._generate_scored_elements(
            120, 0.50, 0.79, ["button", "link"], ["Sort", "Filter", "View Profile", "Settings"], {"affects_view": True}
        )

        # Low importance (0.10-0.49) - 80 samples
        low = self._generate_scored_elements(
            80, 0.10, 0.49, ["label", "image", "view"], ["Copyright", "Decorative", "Spacer"], {"decorative": True}
        )
    ...
    def _generate_scored_elements(
        self, count: int, min_score: float, max_score: float, types: List[str], labels: List[str], properties: Dict

...
@click.option("--screenshots", "-s", type=click.Path(exists=True), help="Path to screenshots directory")
@click.option("--repo", type=click.Path(exists=True), default=".", help="Repository path")
@click.option("--min-confidence", "-c", default=0.7, type=float, help="Minimum confidence threshold (0.0-1.0)")
def analyze(test_results_path: str, screenshots: Optional[str], repo: str, min_confidence: float) -> None:
    ...
    try:
        ...
        orchestrator = HealingOrchestrator(repo_path=repo_path, min_confidence=min_confidence)
...
@click.option("--min-confidence", "-c", default=0.7, type=float, help="Minimum confidence threshold (0.0-1.0)")
def auto(
    test_results_path: str, screenshots: Optional[str], repo: str, commit: bool, dry_run: bool, min_confidence: float
) -> None:
...
def revert(commit_hash: str, repo: str) -> None:
    ...
    try:
        ...
        result = subprocess.run(
            ["git", "revert", commit_hash, "--no-edit"], cwd=repo_path, capture_output=True, text=True
        )

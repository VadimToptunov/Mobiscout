...
def open_deeplink(driver: Any, url: str, platform: str = "android", package: Optional[str] = None) -> Any:
...
def background_app(driver: Any, seconds: float = -1) -> Any:
    """Send the app to the background for ``seconds`` (-1 = leave it there)."""
    return driver.background_app(seconds)


def terminate_app(driver: Any, app_id: str) -> Any:
    return driver.terminate_app(app_id)


def activate_app(driver: Any, app_id: str) -> Any:
    return driver.activate_app(app_id)


# ---- device conditions ------------------------------------------------------
def set_location(driver: Any, latitude: float, longitude: float, altitude: float = 0.0) -> Any:
    """Mock the GPS location (point). Feed a sequence for a route."""
    return driver.set_location(latitude, longitude, altitude)


def set_orientation(driver: Any, orientation: str = "LANDSCAPE") -> None:
    driver.orientation = orientation.upper()


def set_clipboard(driver: Any, text: str) -> Any:
    return driver.set_clipboard_text(text)


def get_clipboard(driver: Any) -> Any:
    return driver.get_clipboard_text()


def set_network(driver: Any, mode: str, provider: Provider = Provider.LOCAL) -> Any:
...
def press_key(driver: Any, key: str) -> Any:

...
class UniversalModelBuilder:
    ...
    def generate_universal_dataset(
        self, samples_per_type: int = 200, output_path: Path = Path("ml_models/universal_training_data.json")
    ) -> Path:
        """
    ...
    def _create_sample_variation(self, template: Dict[str, Any], element_type: ElementType) -> Dict[str, Any]:
...
def create_universal_pretrained_model():
    ...
    metrics = builder.train_universal_model(
        dataset_path=dataset_path, output_model_path=Path("ml_models/universal_element_classifier.pkl")
    )

...
class AllureGenerator:
    ...
    def _generate_test_result(self, test: TestResult, suite: TestSuite, output_dir: Path) -> None:
        ...
        if status == "failed" and test.error_message:
            result["statusDetails"] = {"message": test.error_message, "trace": test.stack_trace or ""}
        elif status == "skipped" and test.error_message:
            result["statusDetails"] = {"message": test.error_message}

        # Add screenshots as attachments
        if test.screenshots:
            result["attachments"] = [
                {"name": Path(screenshot).name, "source": screenshot, "type": "image/png"}

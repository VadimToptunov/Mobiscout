...
`observe crawl` autonomously walks a *running* app and writes the kit:
...
sleep 4
observe crawl --package <pkg> --targets python_pytest,java_testng --output /tmp/kit
```
...
observe crawl --platform ios --package <bundleId> --udid <UDID> --targets python_pytest --output /tmp/ios-kit
```
...
- Set `OBSERVE_ML_AUTOTRAIN=0` to force the heuristic (skip model training) for deterministic/fast runs.
